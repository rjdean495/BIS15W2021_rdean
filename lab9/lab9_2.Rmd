...
penguins %>% 
  filter(sex!="NA") %>% 
  group_by(sex) %>% 
  summarize(ave_bill_length=mean(bill_length_mm), .groups="keep")
```
...
homerange %>% 
  ...
  summarize(min_log10.mass=min(log10.mass),
            ...
            median_log10.mass=median(log10.mass), .groups="keep")
```
...
homerange %>% 
  ...
  summarize(median.mass=median(log10.mass),
            min.mass=min(log10.mass),
            max.mass=max(log10.mass),
            mean.mass=mean(log10.mass),
            .groups="keep")
```
...
homerange %>% 
  filter(trophic.guild=="carnivore", taxon=="mammals") %>% 
  select(family, log10.mass) %>% #not really necessary
