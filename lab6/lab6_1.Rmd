...
msleep %>% 
  group_by(order) %>% 
  summarize(mean.brainwt=mean(brainwt,na.rm=T),.groups= 'keep')
```

2. What does `NA` mean? How are NA's being treated by the summarize function?
NA means that there are a few variables in that data frame that did not have a value and were inputed as NA.

3. Try running the code again, but this time add `na.rm=TRUE`. What is the problem with Cetacea? Compare this to Carnivora. 
```{r}
msleep %>% 
  group_by(order) %>% 
  summarise(mean.brainwt=mean(brainwt,na.rm=T))
```
```{r}
msleep %>% 
